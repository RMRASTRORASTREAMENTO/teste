var IconToSvg = function () {
     var _ = { folderPath: '/ready_uploads/svg/' }; _.init = function () { }; _.getHtml = function (icon, classes, styles) {
          if (!IsIE11()) {
               var brandFlolder = _.getBrandFolder(icon); icon = icon.replace('fa-', ''); icon = icon.replace('fa ', ''); icon = icon.replace('site123-image-icon', ''); icon = icon.replace('site123-svg-icons', ''); // existing customers, we now use `system-svg-icons`
               icon = icon.replace('system-svg-icons', ''); icon = icon.replace('brand-1', ''); icon = $.trim(icon); var url = $GLOBALS["cdn-images-files"] + _.folderPath + brandFlolder + icon + '.svg?v=2'; url = setImagesCDN(url); return '<i data-icon-name="' + icon + '" class="svg-m ' + classes + ' s123-icon-converter" style="' + styles + ' mask: url(\'' + url + '\'); -webkit-mask: url(\'' + url + '\');" alt="' + icon + '">&nbsp;</i>';
          } else { return '<i class="fa site123-image-icon fa-' + icon + ' ' + classes.replace('svg-m', '') + '" alt="' + icon + '"></i>'; }
     }; _.getBrandFolder = function (url) {
          var brandFlolder = ''; if (url.indexOf('brand-1') != -1) { brandFlolder = 'brand-1/'; }
          return brandFlolder;
     }
     function IsIE11() { return !!window.MSInputMethodContext && !!document.documentMode; }
     return _;
}(); function Forms_GoogleRecaptcha() {
     var that = this; that.init = function ($form) {
          that.isActive = false; if ($('#w').val().length != 0) return; that.$form = $form; that.$inputs = that.$form.find('input, textarea, select'); that.$recaptchaToken = that.$form.find('input[name="recaptchaToken"]'); window.isGoogleRecaptchaLoaded = false; that.isGotToken = false; that.isActive = true; that.$inputs.on('focus', function () {
               var isUsingMagicButtons = $('html[dir="ltr"] .all-magic-buttons .m-btn-c').length > 0; if (!window.isGoogleRecaptchaLoaded) {
                    window.isGoogleRecaptchaLoaded = true; $.getScript('https://www.google.com/recaptcha/api.js?render=6Lck3r0ZAAAAAOFc__oZANv72nZ3K29O-qsOIYPp').done(function () {
     ...
     that.getToken = function () {
          try {
               grecaptcha.ready(function () {
                    ...
                    .then(function (token) { that.$recaptchaToken.val(token); that.isGotToken = true; that.$form.submit(); });
               });
          } catch (err) { that.$recaptchaToken.val(''); that.isGotToken = true; that.$form.submit(); }
     }
     that.reset = function () { that.isGotToken = false; window.isGoogleRecaptchaLoaded = false; };
} const ColorsDetector = function () {
     const _ = {}; _.isLightDarkColor = function (c) {
     ...
     return _;
}(); var S123 = function () { var that = {}; that.init = function () { S123.loadDeferCSS(); S123.contextmMenuDisable(); S123.addIE11SupportClass(); }; return that; }(); S123.loadDeferCSS = function () { $(document).on('s123.page.load', function (event) { var $defer_css = $('.defer-css'); $defer_css.each(function () { var $css = $(this); $css.attr('href', $css.attr('data-href')); $css.removeAttr('data-href'); }); }); }; S123.isWebsiteInSlidingWindow = function () {
     try { if (!window.frameElement) return false; } catch (e) { return false; }
     if (window.frameElement.id === 'pagePopupWinID_iFrame') return true;
}(); S123.inIframe = function () { try { return window.self !== window.top; } catch (e) { return true; } }(); S123.addIE11SupportClass = function () { if (!IsIE11()) return; $('html').addClass('ie11-support'); }; function initS123QueryString() {
     S123.QueryString = (function (paramsArray) {
          let params = {}; for (let i = 0; i < paramsArray.length; ++i) {
...
initS123QueryString(); S123.escapeHtml = function (text) { if (!text) return text; var map = { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&apos;' }; return text.toString().replace(/[&<>"']/g, function (m) { return map[m]; }); }; S123.contextmMenuDisable = function () { return; $('body').on('contextmenu', '.disable-context-menu', function () { return false; }); }; S123.objectAssign = function (target, sources) {
     ...
     var _ = { folderPath: '/ready_uploads/svg/' }; _.init = function () {
          $(document).on('s123.page.ready.s123IconToSvg', function (event) {
               if (IsIE11()) {
                    loadFiles(); $('.s123-icon-converter').each(function () {
                         var $this = $(this); var $newIcon = $(_.getHtml($this.data('icon-name'), $this.data('ie11-classes'), '')); if ($this.closest('#top-menu-mobile').length > 0) { var newElem = $('<span class="fa fa-' + $this.data('icon-name') + '">&nbsp;</span>'); $this.replaceWith(newElem); } else {
     ...
     }; _.handleFroalaIcons = function () { loadFiles(); }; _.getHtml = function (icon, classes, styles) { return IconToSvg.getHtml(icon, classes, styles); }; function loadFiles() { if ($('.f-a-css').length > 0) return; $('head').append('<link rel="stylesheet" class="f-a-css" href="../styles/font-awesome.min.css">'); }
     return _;
}(); S123.CopyToClipboard = function () {
     var _ = {}; _.copy = (copyText) => { if (window.isSecureContext && navigator.clipboard) { secureCopy(copyText); } else { unsecuredCopy(copyText); } }
     async function secureCopy(copyText) {
          copyText.select(); copyText.setSelectionRange(0, 99999); try {
...
var menuScrollOffset_mobile = 60; var isMobile = { Android: function () { return navigator.userAgent.match(/Android/i); }, BlackBerry: function () { return navigator.userAgent.match(/BlackBerry/i); }, iOS: function () { return navigator.userAgent.match(/iPhone|iPad|iPod/i); }, Opera: function () { return navigator.userAgent.match(/Opera Mini/i); }, Windows: function () { return navigator.userAgent.match(/IEMobile/i); }, any: function () { return (isMobile.Android() || isMobile.BlackBerry() || isMobile.iOS() || isMobile.Opera() || isMobile.Windows()); } }; var whatScreen = {
     any: function () {
          var screenWidth = $(window).width(); if (screenWidth <= 544) { return 'mobile'; }
...
S123.Jarallax = function () {
     var _ = {}; _.init = function () { $(document).on('s123.page.ready', function (event) { $('.parallax-window').each(function () { _.set($(this)); _.refreshParallaxImages(); }); }); }; _.set = function ($jarallax) {
          if ($jarallax.css('filter')) { $jarallax.data('j-filter', $jarallax.css('filter')); $jarallax.css('filter', 'none'); }
          var options = { speed: isMobile.any() ? 0.75 : 0.5 }; if ($jarallax.data('background-position')) { options.imgPosition = $jarallax.data('background-position'); }
          ...
          if ($jarallax.data('j-filter')) { $jarallax.find('.jarallax-container > div').css({ filter: $jarallax.data('j-filter') }); }
     }; _.refresh = function ($jarallax) { $('.parallax-window').each(function () { jarallax(this, 'onScroll'); jarallax(this, 'onResize'); }); }; _.refreshParallaxImages = function ($jarallax) { $(document).on('s123.page.ready.refreshParallaxImages', function (event) { _.refresh(); }); }; return _;
}(); function Parallax_active(active) {
     return; if (active) {
          if ($('html').hasClass('parallax-active')) return; $('html')
               .addClass('parallax-active')
               .removeClass('parallax-disabled');
     } else {
          if ($('html').hasClass('parallax-disabled')) return; $('html')
...
function intializePopupVideoPlayers($el) {
     $el.off('click.intializePopupVideoPlayers').on('click.intializePopupVideoPlayers', function (e) {
          var $this = $(this); if (!$this.data('video-popup')) return; var player = $this.data('player'); var videoURL = $this.attr('href'); e.preventDefault(); e.stopPropagation(); if (player === 'site123') { videoURL = '/include/globalVideoPlayer.php?cad=1&url=' + encodeURIComponent(videoURL) + '&fluid=true'; }
...
function ContactFormHomeInitialize() {
     $(document).on('s123.page.ready', function (event) {
          if ($('#contactUsFormHome').length !== 0) {
               var $contactUsFormHome = $('#contactUsFormHome'); var clickAction = $contactUsFormHome.data('click-action'); $contactUsFormHome.append($('<div class="conv-code-container"></div>')); var $convCodeContainer = $contactUsFormHome.find('.conv-code-container'); var customFormMultiSteps = new CustomFormMultiSteps(); customFormMultiSteps.init({ $form: $contactUsFormHome, $nextButton: $contactUsFormHome.find('.next-form-btn'), $submitButton: $contactUsFormHome.find('.submit-form-btn'), $previousButton: $contactUsFormHome.find('.previous-form-btn'), totalSteps: $contactUsFormHome.find('.custom-form-steps').data('total-steps') }); var forms_GoogleRecaptcha = new Forms_GoogleRecaptcha(); forms_GoogleRecaptcha.init($contactUsFormHome); $contactUsFormHome.validate({
                    errorElement: 'div', errorClass: 'help-block', focusInvalid: true, ignore: ':hidden:not(.custom-form-step:visible input[name^="datePicker-"])', highlight: function (e) { $(e).closest('.form-group').removeClass('has-info').addClass('has-error'); }, success: function (e) { $(e).closest('.form-group').removeClass('has-error'); $(e).remove(); }, errorPlacement: function (error, element) {
                         if (element.is('input[type=checkbox]') || element.is('input[type=radio]')) { var controls = element.closest('div[class*="col-"]'); if (controls.find(':checkbox,:radio').length > 0) element.closest('.form-group').append(error); else error.insertAfter(element.nextAll('.lbl:eq(0)').eq(0)); }
                         else if (element.is('.select2')) { error.insertAfter(element.siblings('[class*="select2-container"]:eq(0)')); }
                         else if (element.is('.chosen-select')) { error.insertAfter(element.siblings('[class*="chosen-container"]:eq(0)')); }
                         else { error.appendTo(element.closest('.form-group')); }
                    }, submitHandler: function (form) {
                         var $form = $(form); if ($form.hasClass('custom-form') && !CustomForm_IsLastStep($form)) { $form.find('.next-form-btn:visible').trigger('click'); return false; }
                         ...
                         $.ajax({
                              type: "POST", url: url, data: $form.serialize(), success: function (data) {
                                   var dataObj = jQuery.parseJSON(data); $form.trigger("reset"); if (clickAction == 'thankYouMessage' || clickAction == '') { bootbox.alert({ title: translations.sent, message: translations.ThankYouAfterSubmmit + '<iframe src="/versions/' + $('#versionNUM').val() + '/include/contactSentO.php?w=' + $('#w').val() + '&websiteID=' + dataObj.websiteID + '" style="width:100%;height:30px;" frameborder="0"></iframe>', className: 'contactUsConfirm', buttons: { ok: { label: translations.Ok } }, backdrop: true }); } else {
                                        if (dataObj.conv_code.length > 0) { var $convCode = $('<div>' + dataObj.conv_code + '</div>'); $convCodeContainer.html($convCode.text()); }
                                        if (top.$('#websitePreviewIframe').length) { bootbox.alert({ title: translations.previewExternalLinkTitle, message: translations.previewExternalLinkMsg.replace('{{externalLink}}', '<b>' + dataObj.action.url + '</b>'), className: 'externalAlert' }); } else { window.open(dataObj.action.url, '_self'); }
                                   }
                                   customFormMultiSteps.reset(); forms_GoogleRecaptcha.reset(); S123.ButtonLoading.stop($form.find('button:submit')); $form.find('button:submit').prop('disabled', false); WizardNotificationUpdate();
                              }
                         }); return false;
                    }
               }); $contactUsFormHome.find('.f-b-date-timePicker').each(function () { var $option = $(this); var $datePicker = $option.find('.fake-input.date-time-picker'); var $hiddenInput = $option.find('[data-id="' + $datePicker.data('related-id') + '"]'); var $datePickerIcon = $option.find('.f-b-date-timePicker-icon'); var formBuilderCalendar = new calendar_handler(); $datePicker.data('date-format', $contactUsFormHome.data('date-format')); formBuilderCalendar.init({ $fakeInput: $datePicker, $hiddenInput: $hiddenInput, $fakeInputIcon: $datePickerIcon, type: 'datePicker', title: translations.chooseDate, calendarSettings: { format: $datePicker.data('date-format'), weekStart: 0, todayBtn: "linked", clearBtn: false, language: languageCode, todayHighlight: true }, onSubmit: function (selectedDate) { $datePicker.html(selectedDate); $hiddenInput.val(selectedDate); } }); }); CustomForm_DisableTwoColumns($contactUsFormHome);
          }
     });
}
...
function MailingModuleInitialize() {
     if ($('.widget_subscribe_form').length !== 0) {
          var $widget_subscribe_form = $('.widget_subscribe_form'); $widget_subscribe_form.each(function (index) {
               $(this).validate({
                    errorElement: 'div', errorClass: 'help-block', focusInvalid: true, ignore: "", highlight: function (e) { $(e).closest('.form-group').removeClass('has-info').addClass('has-error'); }, success: function (e) { $(e).closest('.form-group').removeClass('has-error'); $(e).remove(); }, errorPlacement: function (error, element) { error.appendTo(element.closest('.form-group')); }, submitHandler: function (form) {
                         var $form = $(form); var $userEmail = $form.find('input[name="widget-subscribe-form-email"]'); var websiteID = $form.find('input[name="websiteID"]').val(); var w = $form.find('input[name="w"]').val(); $form.find('button:submit').prop('disabled', true); $.ajax({
                              type: 'POST', url: '/versions/' + $('#versionNUM').val() + '/include/subscribe.php', data: $form.serialize(), success: function (respondedMessage) {
                                   var respondedMessage = tryParseJSON(respondedMessage); if (!respondedMessage) { $form.find('button:submit').prop('disabled', false); return; }
...
function OpenSearchWindow(closeLocation, customPlaceholder) {
     ...
     }); $('#popupFloatDivSearch .popupCloseButton').on('click', function () { window.history.replaceState('', 'Title', currentPageUrl); }); $('#popupFloatDivSearch .cover').on('click', function () { window.history.replaceState('', 'Title', currentPageUrl); });
}
...
function SearchModuleInitialize() { $(document).on('s123.page.ready.search', function (event) { var $widget_search = $('.widget_search'); $widget_search.each(function () { var $form = $(this); $form.validate({ errorElement: 'div', errorClass: 'help-block', focusInvalid: true, ignore: "", highlight: function (e) { $(e).closest('.form-group').removeClass('has-info').addClass('has-error'); }, success: function (e) { $(e).closest('.form-group').removeClass('has-error'); $(e).remove(); }, errorPlacement: function (error, element) { error.appendTo(element.closest('.form-group')); }, submitHandler: function (form) { OpenSearchWindow('', $form.find('[name="widget-search-form-keyword"]').attr('placeholder')); OpenSearchWindowSearchAjax($form, $form.find('[name="widget-search-form-keyword"]').val()); return false; } }); }); }); }
...
function HomepageVideoSettingInitialize() {
     $(document).on('s123.page.ready', function (event) {
          if ($('#homepage_full_screen_3_party_video').length !== 0) {
               var $videoIframe = $('#homepage_full_screen_3_party_video'); if ($videoIframe[0].src.indexOf("youtube.com") > -1) {
                    (function () {
                         var script = document.createElement('script'); script.src = "http://www.youtube.com/player_api";
                    ...
                    (function () {
                         var script = document.createElement('script'); script.src = "https://f.vimeocdn.com/js/froogaloop2.min.js";
...
function ActivePopupActionButtonsInPage() {
     $('[data-toggle="search_menuCallActionIcons"]').off('click').click(function () { var $this = $(this); OpenSearchWindow($this.data('closeLocation'), websiteCustomSearchPlaceHolder); }); $('[data-toggle="social_menuCallActionIcons"]').off('click').click(function () {
          var $this = $(this); if (findBootstrapEnvironment() == 'xs') { var isMobile = 'mobile'; } else { var isMobile = ''; }
          var content = $('#header-social-content').html(); buildPopup('popupFloatDivSearch', '', content, '', true, true, true, $this.data('closeLocation'), ''); $(document).trigger('s123.page.ready.wizard_preview_manage_helpers');
     }); $('[data-toggle="phone_menuCallActionIcons"]').off('click').click(function () {
          var $this = $(this); var $header_phone_content = $('#header-phone-content').clone(); var multiPhonesObj = tryParseJSON($('#multiPhonesSettings').val()); if (!multiPhonesObj) return; if (multiPhonesObj.length == 1 && multiPhonesObj[0].note == "") {
     ...
     }); S123.globalContactEmail.init(); $('[data-toggle="address_menuCallActionIcons"]').off('click').click(function () { var $this = $(this); buildPopup('popupFloatDivSearch', '', $('#header-address').html(), '', true, true, true, $this.data('closeLocation'), ''); $(document).trigger('s123.page.ready.wizard_preview_manage_helpers'); });
}
function Site123AdButtonInitialize() {
     var $html; var $showSmallAdOnScroll; var banner_height; if (IsWizard()) { $('#showSmallAdOnScroll').addClass('hidden'); return; }
     $(document).on('s123.page.ready', function (event) {
          $html = $('html'); $showSmallAdOnScroll = $('#showSmallAdOnScroll'); if ($showSmallAdOnScroll.length === 0) return; if ($showSmallAdOnScroll.hasClass('static')) { addHtmlClass(); return; }
          isSmallAds(); banner_height = $('#showSmallAdOnScroll').outerHeight(); bannerHandler(); $(window).scroll(function () { bannerHandler(); });
     }); $(document).on('s123.page.ready.isSmallAds', function (event) { isSmallAds(); }); function bannerHandler() { var offset = ($html.hasClass('inside_page') || $html.hasClass('rich_page')) ? 0 : 50; if ($(window).scrollTop() >= offset) { addHtmlClass(); $showSmallAdOnScroll.css({ bottom: '0' }); } else { removeHtmlClass(); $showSmallAdOnScroll.css({ bottom: (-1 * banner_height) }); } }
...
function GetMenuPosition() {
     $(document).on('s123.page.ready', function (event) {
          layoutMenuPositionTXT = $('#layoutMenuPositionTXT').val(); layoutMenuPositionOpenMenuTXT = ChangeDirection(layoutMenuPositionTXT); if (layoutMenuPositionTXT == 'left' || layoutMenuPositionTXT == 'right') { FixMenuTopPosition_SideMenu(); }
...
function ScrollToModule(fromModuleID, toModuleID) {
     var offset = findBootstrapEnvironment() != 'xs' ? menuScrollOffset : menuScrollOffset_mobile; var $scrollTo = $('#section-' + toModuleID)
...
function activeDropDownMenusAction() {
     $('.dropdown-submenu > a').off('click.activeDropDownMenusAction').on('click.activeDropDownMenusAction', function (event) { if ($(this).parent().data('is-eCommerce')) { event.preventDefault(); } }); $('.navPages li').find('a').off('mouseenter.hideHoverMenu'); $('.navPages, #top-menu .navActions').find('.dropdown-submenu')
          ...
          .on('click.subMenu mouseenter.subMenu mouseover.subMenu mouseout.subMenu mouseleave.subMenu', function (e) {
               var $this = $(this).find('> a'); var eventType = e.type; if (eventType == 'mouseenter') { activeDropDownMenusAction_open(e, $this); }
...
function elementInViewport(el) {
     if (!el) return; var top = el.offsetTop; var left = el.offsetLeft; var width = el.offsetWidth; var height = el.offsetHeight; while (el.offsetParent) { el = el.offsetParent; top += el.offsetTop; left += el.offsetLeft; }
     ...
     if (findBootstrapEnvironment() != 'xs' && CheckMenuWidthSpace($place) && $nav.find('>li>a').length > 1) {
          if (CheckMenuWidthSpace($place)) {
               if ($nav.find('.extra-nav-more').length == 0) {
                    var x = '<li class="moduleMenu extra-nav-more dropdown-submenu"><a href="#" aria-haspopup="true" aria-expanded="true"><span class="txt-container">' + translations.more.toLowerCase() + '</span>'; if ($place == 'footer') { x += ' ' + S123.s123IconToSvg.getHtml('caret-up', '', '') + '</a> <ul class="site-dropdown-menu dropdown-side-open-up'; } else { x += ' ' + S123.s123IconToSvg.getHtml('caret-down', '', '') + '</a> <ul class="site-dropdown-menu'; }
                    x += '"></ul></li>'; $nav.append(x);
               }
               var $newLIpage = $nav.find(">li").eq(-2).detach().prependTo($nav.find('.extra-nav-more>ul')); if ($newLIpage.hasClass('dropdown-submenu') == true) {
...
function CheckMenuWidthSpace($place) {
     if ($place == 'header') {
          switch ($('#layoutNUM').val()) {
               case '2': if ($('#mainNav .site_container').width() - 50 < $('#top-menu .navPages').outerWidth(true) + $('#top-menu .navActions').outerWidth(true)) { return true; } else { return false; }
     ...
     if ($place == 'footer') {
          switch ($('#footer_layout').val()) {
               case '2': if ($('.global_footer .part1').outerWidth(true) - 100 < $('.global_footer .nav').outerWidth(true)) { return true; } else { return false; }
...
function ReduseMenuSizeWhenWeDontHavePlaceHeight_action() {
     var $nav = $('#top-menu .navPages'); if (findBootstrapEnvironment() != 'xs' && CheckMenuWidthSpaceHeight() && $nav.find('>li>a').length > 1) {
          if (CheckMenuWidthSpaceHeight()) {
               if ($nav.find('.extra-nav-more').length == 0) {
                    var x = '<li class="moduleMenu extra-nav-more dropdown-submenu"><a href="#" aria-haspopup="true" aria-expanded="true">'; if ($('html').attr('dir') == 'rtl') { x += '<span class="txt-container">' + translations.more.toLowerCase() + '</span>'; x += ' ' + S123.s123IconToSvg.getHtml('caret-left', '', ''); x += '</a> <ul class="site-dropdown-menu dropdown-side-open-left"></ul></li>'; } else { x += '<span class="txt-container">' + translations.more.toLowerCase() + '</span>'; x += ' ' + S123.s123IconToSvg.getHtml('caret-right', '', ''); x += '</a> <ul class="site-dropdown-menu dropdown-side-open-right"></ul></li>'; }
                    $nav.append(x);
               }
...
function ShowMenuAfterReduseSize($place) {
     if (IsOnlyContent()) return; if ($('#top-menu').length > 0 && $('#layoutNUM').val() == '21' && $place == 'header') {
          $('#centerLogo19').remove(); $('#top-menu').css({ 'padding-right': '0', 'padding-left': '0' }); var menuWidth = ($('#top-menu .navPages').outerWidth(true) + $('#top-menu .navActions').outerWidth(true)) / 2; var sumLIofMenu = 0; var saveLIplace = 1; var extraPaddingFromSideOne = 0; $('#top-menu .navPages > li').each(function () {
               var $this = $(this); sumLIofMenu += $this.outerWidth(true); if (sumLIofMenu >= menuWidth) { extraPaddingFromSideOne = sumLIofMenu - menuWidth; return false; }
               saveLIplace++;
          ...
          (function () {
               var $logo = $('#centerLogo19 a'); var href = $logo.attr('href'); $logo
          ...
          })(); ShowMenuAfterReduseSize_finishCalc(); ShowMenuAfterReduseSize_finishCalc(); ShowMenuAfterReduseSize_finishCalc(); ShowMenuAfterReduseSize_finishCalc(); if (IsWizard()) { $(document).trigger('centerLogo19.added'); }
     }
     if ($('#header').length == 0 && $('#top-menu').length > 0 && $place == 'header') {
          var rectMenu = 0; var rectHeader = Math.round($('#mainNav .site_container').width()); $('#mainNav .site_container').children().each(function () { rectMenu += Math.round(this.offsetWidth); }); var $replaceActionButtonsToIcon = $('.replaceActionButtonsToIcon').length > 0 ? $('.replaceActionButtonsToIcon') : $('<li class="header-menu-wrapper replaceActionButtonsToIcon dropdown-submenu"><a data-close-location="left" class="btn" role="button" data-container="body" data-toggle="menuCallActionIcons">' + S123.s123IconToSvg.getHtml('bars', '', '') + '</a><ul class="site-dropdown-menu" data-rel="navActions" style="opacity: 1;"></ul></li>'); if ($('#mainNavMobile').is(":visible") == false && rectMenu > rectHeader && $('#top-menu .navActions .header-menu-wrapper').length == 0 && $('#top-menu.affix').length == 0) { $('#top-menu .navActions').append($replaceActionButtonsToIcon); $replaceActionButtonsToIcon.find('.site-dropdown-menu').append($('.action-button-wrapper')); TriggerS123PageReady(); ResetMoreButton(); }
          rectMenu = Math.round($('#top-menu').outerWidth()); rectHeader = Math.round($('#mainNav .site_container').width()); if ($('#mainNavMobile').is(':visible') == false && rectMenu > rectHeader && $('#top-menu .navActions .header-menu-wrapper').length > 0 && $('.replaceActionButtonsToIcon').length > 0 && $('.replaceActionButtonsToIconRemoveExtra').length == 0 && $('#top-menu.affix').length == 0) { $replaceActionButtonsToIcon.find('.site-dropdown-menu').append($('#top-menu').find('.header-phone-wrapper, .header-address-wrapper, .header-social-wrapper, .header-search-wrapper, .header-email-wrapper')); $replaceActionButtonsToIcon.addClass('replaceActionButtonsToIconRemoveExtra'); TriggerS123PageReady(); ResetMoreButton(); }
     }
     ...
     $('#mainNavMobile').css({ 'opacity': '1' }); activeDropDownMenusAction();
}
...
function FixMenuTopPosition_SideMenu() {
     $('.navPages .dropdown-submenu > a').off('click.FixMenuTopPosition mouseenter.FixMenuTopPosition').on('click.FixMenuTopPosition mouseenter.FixMenuTopPosition', function (e) {
          var $this = $(this).parent().find('.site-dropdown-menu'); if ($this.length > 0) {
               setTimeout(function () {
                    var rect = $this[0].getBoundingClientRect(); if (rect.top + rect.height > window.innerHeight && rect.height < window.innerHeight) { $this.css('top', parseInt($this.css('top'), 10) - (rect.top + rect.height - window.innerHeight) - 25); }
                    $this.css('opacity', '1');
               }, 100);
          }
     });
}; function FixMenuTopPosition_TopMenu() {
     var $menuLinks = $('.navPages .dropdown-submenu > a, .global_footer .nav .dropdown-submenu > a'); $menuLinks
          ...
          .on('click.FixMenuTopPosition mouseenter.FixMenuTopPosition', function (e) {
               var $dropdown = $(this).parent().find('> .site-dropdown-menu'); var isCategory = $dropdown.parents('ul').length > 1; if ($dropdown.length === 0) return; setTimeout(function () {
                    if ($dropdown.length === 0) return; var rect = $dropdown[0].getBoundingClientRect(); if (rect.top + rect.height > window.innerHeight && rect.height < window.innerHeight) { $dropdown.css({ 'bottom': '100%', 'top': 'auto' }); } else { if (rect.top < 0 || rect.bottom < 0) { $dropdown.css({ 'top': '100%', 'bottom': 'auto' }); } }
...
function ResetMoreButton() {
     $('#mainNav #top-menu .navPages, #mainNav #top-menu .navActions, #mainNav #top-menu .headerSocial, #header .header-row, .global_footer .nav').css({ 'opacity': '0' }); $('#top-menu .navPages .extra-nav-more > ul > li').each(function () {
          var $this = $(this); if ($('#mainNav #top-menu').length > 0) { $this.find('.site-dropdown-menu').removeClass('dropdown-side-open-left'); if ($('html').attr('dir') == 'rtl') { $this.find('.fa').removeClass('fa-caret-left').addClass('fa-caret-down'); } else { $this.find('.fa').removeClass('fa-caret-right').addClass('fa-caret-down'); } }
...
function openDivMenuOnLanguageClickAction() { var content = '<ul class="languagesList navPagesPopup">'; $.each(languageList, function (index, language) { if (language['countryCode'] && language['countryCode'] != '') { content += '<li><a href="' + language['url'] + '"><img src="/files/vendor/flag-icon-css-master/flags/1x1/' + language['countryCode'] + '.svg" style="width:20px;height:14px;">&nbsp;' + language['name'] + '</a></li>'; } else { content += '<li><a href="' + language['url'] + '">' + language['name'] + '</a></li>'; } }); content += '</ul>'; buildPopup('popupFloatDivMenuLanguages', '', content, '', true, true, true, '', ''); }
function PageScrollByClick() { $(document).on('s123.page.ready.pageScrollByClick', function (event) { var offset = findBootstrapEnvironment() != 'xs' ? menuScrollOffset : menuScrollOffset_mobile; $('a.page-scroll').off('click.scrollEvent').on('click.scrollEvent', function (event) { var $anchor = $(this); $('html, body').stop().animate({ scrollTop: ($($anchor.attr('href')).offset().top - offset) }, 1250, 'easeInOutExpo'); event.preventDefault(); }); }); }
...
function findBootstrapColPerRow($items) { if (!$items || $items.length === 0) return 0; var first_item_offset_top = $items.first().offset().top; var col_per_row = 0; $items.each(function () { if ($(this).offset().top === first_item_offset_top) { col_per_row += 1; } else { return; } }); return col_per_row; }
...
function buildSmallPopup_CloseAction(popID) {
     var $popup = $('#' + popID); if (popID == 'popupCart' && window.location.pathname == '/-order1/') { $(document).trigger('order1.reload'); }
...
function buildPopup(popID, title, content, iframeURL, closeEsc, closeEnter, oneColor, closeLocation, customClasses) {
     if ($('#' + popID).length !== 0) return; if (iframeURL != '') { var iClass = ''; if (iframeURL.indexOf("youtube.com") > -1) iClass = 'videoSize'; if (iframeURL.indexOf("vimeo.com") > -1) iClass = 'videoSize'; content = '<iframe id="' + popID + '_iFrame" src="' + iframeURL + '" class="iframe ' + iClass + '" allowfullscreen></iframe>'; }
...
S123.VideoHandler = function () {
     var _ = {}; _.init = function () {
          $('.s123-video-handler:not([data-is-full-screen="true"])').each(function () { _.addClickEvent($(this)); }); if (isMobile.any()) { $('.s123-video-handler:not([data-player="youtube"]):not([data-is-full-screen="true"])').imagesLoaded().progress(function (instance, image) { s123VideoHandler($(image.img).closest('.s123-video-handler')); }); }
          $('.s123-video-handler[data-is-full-screen="true"]').on('click.VideoHandler', function (event) {
               var $this = $(this); var src = $this.data('video'); $.magnificPopup.open({
                    mainClass: 'mfp-e-product-gallery', delegate: '.e-p-mfp-image', 		// Categories Filter
                    closeOnContentClick: true, closeBtnInside: false, tLoading: translations.loading, 	// Text that is displayed during loading
                    type: 'iframe', items: { src: src }, iframe: {
                         markup: '<div class="mfp-iframe-scaler">' + '<div class="mfp-close"></div>' + '<iframe class="mfp-iframe" frameborder="0" allowfullscreen></iframe>' + '<div class="mfp-title" style="position: absolute; padding-top: 5px;"></div>' + '</div>', patterns: {
                              youtube: {
                                   index: 'youtube.com/', id: function (url) { var matches = url.match(/[\\?\\&]v=([^\\?\\&]+)/); if (!matches || !matches[1]) return null; return matches[1]; }, src: '//www.youtube.com/embed/%id%?autoplay=1'
                              }, vimeo: {
                                   index: 'vimeo.com/', id: function (url) { var matches = url.match(/(https?:\/\/)?(www.)?(player.)?vimeo.com\/([a-z]*\/)*([0-9]{6,11})[?]?.*/); if (!matches || !matches[5]) return null; return matches[5]; }, src: '//player.vimeo.com/video/%id%?autoplay=1'
                              }, site123: { index: $GLOBALS['cdn-user-files'], id: function (url) { if (isMobile.any()) url += '&autoplay=0'; return url; }, src: '/include/globalVideoPlayer.php?cad=1&url=%id%' }, site123Processing: { index: '/files/images/video-processing.png', id: function (url) { if (isMobile.any()) url += '&autoplay=0'; return url; }, src: '/include/globalVideoPlayer.php?cad=1&url=%id%' }
                         }
                    },
               });
          });
     }; _.addClickEvent = function ($el) { $el.on('click.VideoHandler', function () { s123VideoHandler($(this)); }); }; return _;
}(); function s123VideoHandler($obj) {
     var player = $obj.data('player'); var video_id = $obj.data('video-id'); var videoURL = $obj.data('video'); var customStyle = $obj.find('img').attr('style') ? $obj.find('img').attr('style') : ''; var width = $obj.find('img').width(); var height = $obj.find('img').height(); var originalHtml = $obj.prop('outerHTML'); if (player === 'site123') { videoURL = '/include/globalVideoPlayer.php?cad=1&url=' + encodeURIComponent(videoURL) + '&width=' + width + '&height=' + height; }
     ...
     var $iframe = $('<div class="video-wrapper"><iframe id="v-h-' + video_id + '" data-player="' + player + '" style="' + customStyle + 'width:' + width + 'px;height:' + height + 'px;" type="text/html" src="' + videoURL + '" allow="autoplay; fullscreen" frameborder="0" allowfullscreen></iframe></div>'); $iframe.data('original-el', originalHtml); $obj.replaceWith($iframe); $(document).one('S123Resize.start.VideoHandler', function () { var $el = $($iframe.data('original-el')); $el.css('visibility', 'hidden'); $iframe.replaceWith($el); $el.imagesLoaded().done(function (instance, image) { $el.css('visibility', 'visible'); }); S123.VideoHandler.addClickEvent($el); }); (function () {
          if (player !== 'youtube') return; if (!isMobile.any()) return; var youtube_player; if ($('#youtube_player_api').length === 0) {
               var script = document.createElement('script'); script.id = 'youtube_player_api'; script.src = "https://www.youtube.com/iframe_api";
               var firstScriptTag = document.getElementsByTagName('script')[0]; firstScriptTag.parentNode.insertBefore(script, firstScriptTag); window.onYouTubePlayerAPIReady = function () { youtube_player = YouTubePlayerInitialize(video_id); }
          } else { youtube_player = YouTubePlayerInitialize(video_id); }
          function onPlayerReady(event) { if (isMobile.iOS()) event.target.mute(); event.target.playVideo(); }
...
function getCouponDetails(callback, couponCode, w, websiteID, versionNUM, total) {
     if (couponCode.length === 0) return; $.ajax({
          type: "POST", url: "/versions/" + versionNUM + "/wizard/orders/front/getCouponsAjax.php", data: 'w=' + w + '&websiteID=' + websiteID + '&couponCode=' + couponCode + '&total=' + total, success: function (data) {
...
function getScrollbarWidth() {
     if ($(document).height() > $(window).height()) { //Make sure this page have a scroll
          ...
          document.body.appendChild(outer); var widthNoScroll = outer.offsetWidth; outer.style.overflow = "scroll"; var inner = document.createElement("div"); inner.style.width = "100%"; outer.appendChild(inner); var widthWithScroll = inner.offsetWidth; outer.parentNode.removeChild(outer); return widthNoScroll - widthWithScroll;
     } else {
...
var topWindow = function () {
     var win = window; var top = win; while (win.parent != win) {
          try { win.parent.document; top = win.parent; } catch (e) { }
          win = win.parent;
     }
     return top;
...
function homepageRandomTextAction(t, hasStop) {
     var $this = $(t); var words = $this.data('text'); var counter = 0; var speed = 5000; words = words.split('|'); if (words.length > 0) {
          if (words[words.length - 1].includes('t:')) { var speedEle = words[words.length - 1].replace(/t:(.*)/, '$1'); if ($.isNumeric(speedEle)) { speed = speedEle; words.splice(words.length - 1, 1); } }
          $this.html(words[counter]).addClass('elementToFadeIn'); counter++; var inst = setInterval(function () {
               $this.removeClass('elementToFadeIn'); setTimeout(function () { $this.html(words[counter]).addClass('elementToFadeIn'); }, 50); counter++; if (counter >= words.length) {
                    counter = 0; if (hasStop == 'yes') {
...
$.fn.containerTextWidth_site123 = function (fontSize) { var $this = $(this); if ($this.length === 0) return; var bodyWidth = $('body').width() - 50; var fontFamily = $this.css('font-family').replace('"', '\''); var html_calc = '<div id="containerTextWidth_site123" style="position:absolute;opacity:0;width:' + bodyWidth + 'px;display:table;font-size:' + fontSize + ';letter-spacing: ' + $this.css('letter-spacing') + ';line-height: ' + $this.css('line-height') + ';word-wrap: ' + $this.css('word-wrap') + ';white-space: ' + $this.css('white-space') + ';font-family: ' + fontFamily + ';">' + $this.html() + '</div>'; $('body').append(html_calc); var width = $('#containerTextWidth_site123').width(); $('#containerTextWidth_site123').remove(); return width; }; var s123MobileMenu = new function () {
     ...
     function addPopupCustomEvents() {
          setTimeout(function () {
               var navHeight = $('#popupFloatDivMenu .navPagesPopup').outerHeight(true) + 100; var actionHeight = $('.navPagesPopupActionButtons').outerHeight(true); var screenHeight = $('#popupFloatDivMenu .page').outerHeight(true); if (navHeight + actionHeight > screenHeight) { $('#popupFloatDivMenu .navPagesPopup').height(screenHeight - actionHeight - 15); } else { $('#popupFloatDivMenu .navPagesPopup').height(navHeight - 15); }
               $('#popupFloatDivMenu .navPagesPopup .site-dropdown-menu').css('opacity', '1');
          }, 150); activeDropDownMenusAction(); $('#popupFloatDivMenu .navPagesPopup li').not('.dropdown-submenu').find('a').click(function () { buildPopup_CloseAction('popupFloatDivMenu'); }); $('#popupFloatDivMenu .navPagesPopupActionButtons_part2 a').click(function () { buildPopup_CloseAction('popupFloatDivMenu'); }); ActivePopupActionButtonsInPage(); $(document).trigger('s123.page.ready.pageScrollByClick'); $(document).trigger('s123.page.ready.ActiveLanguageButton');
     }
     ...
     function animate(action, callBack) {
          var animation = { ul: {}, openedCat: {} }; var $categoryPages = that.$selectedCat.find('.active-cat-content'); that.contentOffset = $(window).width(); addRemoveAnimationClass('add', [$categoryPages, that.$container, that.$page, that.$ul, that.$selectedCat]); if (action == 'show') {
               that.$tmpDiv = $('<div style="width:100%;height:' + that.$ul.height() + 'px; margin-bottom:10px;"></div>'); that.$ul.parent().prepend(that.$tmpDiv); if (that.isRtl) {
                    that.$selectedCat.css({ position: 'absolute', top: that.menuTop, right: that.contentOffset, height: that.menuHeight }); animation.openedCat.right = ($(window).width() - (that.$page.offset().left + that.$page.outerWidth())) + 5; that.$ul.css({
                         position: 'absolute', top: that.menuTop, right: ($(window).width() - (that.$page.offset().left + that.$page.outerWidth())) + 5, fontFamily: 'auto', height: that.menuHeight, zIndex: 1 // edge bug fix - element is disappears without z-index
                    }); animation.ul.right = that.contentOffset * -1;
               } else {
                    that.$selectedCat.css({ position: 'absolute', top: that.menuTop, left: that.contentOffset, height: that.menuHeight }); animation.openedCat.left = that.$page.offset().left + 5; that.$ul.css({
                         position: 'absolute', top: that.menuTop, left: that.$page.offset().left + 5, fontFamily: 'auto', height: that.menuHeight, zIndex: 1 // edge bug fix - element is disappears without z-index
               ...
               if (that.isRtl) { animation.ul.right = ($(window).width() - (that.$page.offset().left + that.$page.outerWidth())) + 5; animation.openedCat.right = that.contentOffset; } else { animation.ul.left = that.$page.offset().left + 5; animation.openedCat.left = that.contentOffset; }
     ...
     function generateHTML() {
          ...
          var html = '<ul class="navPagesPopup fancy-scrollbar">' + that.$source.html() + '</ul>'; html += '<div class="navPagesPopupActionButtons">'; html += '<div class="navPagesPopupActionButtons_part1">'; if ($('.header-address-wrapper').length > 0) { html += $('.header-address-wrapper').clone().html(); }
     ...
     function openMenu() {
          var customClass = ''; var addCustomCover = false; switch (that.menuType) { case 0: customClass = ''; break; case 1: customClass = 'side-menu'; addCustomCover = true; break; case 2: customClass = 'side-menu half-width'; addCustomCover = true; break; }
     ...
     function addRemoveAnimationClass(action, elements) {
          switch (action) {
               case 'add': for (var i = 0; i < elements.length; i++) { elements[i].addClass('m-m-progress'); }
                    break; case 'remove': for (var i = 0; i < elements.length; i++) { elements[i].removeClass('m-m-progress'); }
                    break;
          }
     }
     return that;
}(); function moduleLayoutCategories_shadow() { $(document).on('s123.page.ready', function (event) { $.each($('.s123-module .s123-categories'), function () { var $this = $(this); var $categoriesContainer = $this.find('ul'); var $mrsFirst = $('<div class="m-r-s"></div>'); var $mrsLast = $('<div class="m-r-s"></div>'); $this.prepend($mrsFirst); $this.append($mrsLast); if ($this.closest('.s123-module-gallery').length > 0 && !$this.hasClass('col-xs-12')) { $mrsFirst.css({ left: 0 }); $mrsLast.css({ right: 0 }); } }); }); }
var ProgressveWebApp = new function () {
     var that = this; that.init = function () {
     ...
     var that = {}; that.init = function () { that.$container = settings.$container; that.ajax = settings.ajax; that.inProgress = false; that.pageNum = 2; that.initilized = false; that.disabled = false; that.id = settings.id; that.offset = settings.offset; that.loader = settings.loader; that.$loading = $(generateHtml()); that.$container.parent().append(that.$loading); that.addLoadNextPageAbility(); that.initilized = true; that.$container.data('s123InfiniteScroll', that); }; that.addLoadNextPageAbility = function () { $(window).off('scroll.s123InfiniteScroll' + that.id).on('scroll.s123InfiniteScroll' + that.id, function () { if (that.disabled) return; if ($(window).scrollTop() + $(window).height() >= that.$container.height() - that.offset) { that.getPage(); that.inProgress = true; } }); }; that.destroy = function () { $(window).off('scroll.s123InfiniteScroll' + that.id); that.$loading.remove(); if (that.request) that.request.abort(); }; that.getPage = function () {
          if (that.inProgress) return; showLoadingAnimation(); that.ajax.data.pageNumber = that.pageNum; that.request = $.ajax({
               type: that.ajax.type, url: that.ajax.url, data: that.ajax.data, success: function (data) {
                    data = tryParseJSON(data); that.pageNum++; that.inProgress = false; hideLoadingAnimation(); if (!data.hasNextPage) { that.destroy(); }
                    if (that.ajax.success) that.ajax.success.call(this, data); that.request = null;
               }
          });
     }; that.disable = function () { that.disabled = true; }; that.enable = function () { that.disabled = false; }; function generateHtml() { var html = ''; html += '<div class="wizard-pagination text-center" style="display:none; width: 100%; padding: 10px;">'; html += '<div class="loading-icon" style="width:100%;">'; html += '<div class="s123-loader-ellips infinite-scroll-request">'; html += '<span class="s123-loader-ellips__dot"></span>'; html += '<span class="s123-loader-ellips__dot"></span>'; html += '<span class="s123-loader-ellips__dot"></span>'; html += '<span class="s123-loader-ellips__dot"></span>'; html += '</div>'; html += '</div>'; html += '</div>'; return html; }
...
S123.popOver = function () {
     ...
     }; function hide() { that.$el.popover('hide'); $(window).off('mousedown.destroyPopover' + '.' + that.namespace); $(window).off('blur.destroyPopover' + '.' + that.namespace); $(window).off('scroll.destroyPopover' + '.' + that.namespace); }
     ...
     var that = {}; that.init = function () {
          ...
          var $form = $('.g-c-email-form'); var customFormMultiSteps = new CustomFormMultiSteps(); customFormMultiSteps.init({ $form: $form, $nextButton: $form.find('.next-form-btn'), $submitButton: $form.find('.submit-form-btn'), $previousButton: $form.find('.previous-form-btn'), totalSteps: $form.find('.custom-form-steps').data('total-steps') }); $form.find('.f-b-date-timePicker').each(function () { var $option = $(this); var $datePicker = $option.find('.fake-input.date-time-picker'); var $hiddenInput = $option.find('[data-id="' + $datePicker.data('related-id') + '"]'); var $datePickerIcon = $option.find('.f-b-date-timePicker-icon'); var formBuilderCalendar = new calendar_handler(); $datePicker.data('date-format', $form.data('date-format')); formBuilderCalendar.init({ $fakeInput: $datePicker, $hiddenInput: $hiddenInput, $fakeInputIcon: $datePickerIcon, type: 'datePicker', title: translations.chooseDate, calendarSettings: { format: $datePicker.data('date-format'), weekStart: 0, todayBtn: "linked", clearBtn: false, language: languageCode, todayHighlight: true }, onSubmit: function (selectedDate) { $datePicker.html(selectedDate); $hiddenInput.val(selectedDate); } }); }); CustomForm_DisableTwoColumns($form); var forms_GoogleRecaptcha = new Forms_GoogleRecaptcha(); forms_GoogleRecaptcha.init($form); $form.validate({
               errorElement: 'div', errorClass: 'help-block', focusInvalid: true, ignore: ':hidden:not(.custom-form-step:visible input[name^="datePicker-"])', highlight: function (e) { $(e).closest('.form-group').removeClass('has-info').addClass('has-error'); }, success: function (e) { $(e).closest('.form-group').removeClass('has-error'); $(e).remove(); }, submitHandler: function (form) {
                    var $form = $(form); $form.find('button:submit').prop('disabled', true); if (forms_GoogleRecaptcha.isActive && !forms_GoogleRecaptcha.isGotToken) { forms_GoogleRecaptcha.getToken(); return false; }
                    $.ajax({
                         type: "POST", url: "/versions/" + $('#versionNUM').val() + "/include/contactEmailO.php", data: $form.serialize(), success: function (data) {
                              data = tryParseJSON(data); $form.trigger("reset"); if (data.conv_code && data.conv_code.length > 0) { var $convCode = $('<div>' + data.conv_code + '</div>'); $form.find('.conv-code-container').html($convCode.text()); }
                              ...
                              forms_GoogleRecaptcha.reset(); $form.find('button:submit').prop('disabled', false); WizardNotificationUpdate();
                         }
                    }); return false;
...
}(); function previewScaleDeviceTypeChange(deviceType) { $(document).trigger('previewScale.deviceTypeChange', [deviceType]); $(document).trigger('S123PopOver.reposition'); }
function previewReloadPreviewCSSReloaded() { $(document).trigger('reloadPreviewCSS.reloaded'); }
function CustomFormMultiSteps() {
     var that = this; that.init = function (settings) { that.totalSteps = settings.totalSteps; that.$form = settings.$form; that.$nextButton = settings.$nextButton; that.$submitButton = settings.$submitButton; that.$previousButton = settings.$previousButton; that.initMultiStepsButtons(); }; that.initMultiStepsButtons = function () {
          that.$previousButton.off('click').on('click', function () { var $step = that.$form.find('.custom-form-step:visible'); var step = $step.data('step'); var previousStep = step - 1; $step.hide(); that.$form.find('.custom-form-step.step-' + previousStep).fadeIn(350); that.$submitButton.hide(); that.$nextButton.fadeIn(350); if (previousStep < 2) { that.$previousButton.hide(); } else { that.$previousButton.fadeIn(350); } }); that.$nextButton.off('click').on('click', function () {
               var $step = that.$form.find('.custom-form-step:visible'); var step = $step.data('step'); var nextStep = step + 1; if (!that.$form.valid()) return; var offset = findBootstrapEnvironment() != 'xs' ? menuScrollOffset : menuScrollOffset_mobile; var isScrollTopForm = that.$form.offset().top - offset < $(window).scrollTop(); $step.hide(); that.$form.find('.custom-form-step.step-' + nextStep).fadeIn(350); that.$previousButton.css('display', 'block'); if (nextStep >= that.totalSteps) { that.$nextButton.hide(); that.$submitButton.fadeIn(350); }
...
S123.ButtonLoading = function () {
     var that = {}; that.start = function ($button) { if ($button.find('.s123-btn-loading').length > 0) return; var html = $button.html(); $button.html('<div class="s123-btn-loading"><span class="s123-btn-loading-text">' + html + '</span>' + S123.s123IconToSvg.getHtml('spinner', 'fa-spin white', '') + '</div>'); }
     ...
     return that;
}(); function HomepageCountdown() { $(document).on('s123.page.ready', function (event) { var $section = $('#top-section'); var $countdown = $section.find('#homepageCountdown'); var $container = $section.find('.homepage-countdown-container'); if ($countdown.length == 0) return; var countdownWidget = new CountdownWidget(); countdownWidget.init({ $countdown: $countdown, dateInfo: $countdown.data('date-info'), type: $countdown.data('type'), translate: { days: $countdown.data('days'), hours: $countdown.data('hours'), minutes: $countdown.data('minutes'), seconds: $countdown.data('seconds') }, onStop: function () { $container.find('.message').fadeIn(350); } }); }); }
S123.CrossOriginHandler = function () { var _ = {}; _.init = function () { if ($('[data-rel="crossOriginHandler"]').length == 0) return; $(window).on('message.crossOriginHandler', function (event) { var data = event.originalEvent.data; switch (data.eventType) { case 'resize': $('#' + data.id).css({ height: data.height }); break; } }); }; return _; }(); $.fn.popover.Constructor.prototype.reposition = function () {
     var $tip = this.tip(); var autoPlace = true; var placement = typeof this.options.placement === 'function' ? this.options.placement.call(this, $tip[0], this.$element[0]) : this.options.placement; var pos = this.getPosition(); var actualWidth = $tip[0].offsetWidth; var actualHeight = $tip[0].offsetHeight; if (autoPlace) {
